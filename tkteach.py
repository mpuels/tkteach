...
class tkteach:
    ...
    def __init__(self, master):
        ...
        self.datasetTitleLabel = tk.Label(self.frameLEFT,
        ...
        self.loadDataSetButton = tk.Button(self.frameLEFT, text="Load Data Set",
                                           command=self.loadDataSet)
        self.loadDataSetButton.pack()

        self.dataSetStatusLabel = tk.Label(self.frameLEFT,
                                           text="No Data Set Loaded!")
        ...
        self.prevImageButton = tk.Button(self.frameMIDDLEBUTTONS,
                                         text="<- Save & Previous",
                                         command=self.prevImage,
                                         state=tk.DISABLED)
        self.prevImageButton.pack(side=tk.LEFT)

        self.nextImageButton = tk.Button(self.frameMIDDLEBUTTONS,
                                         text="Save & Next ->",
        ...
        self.imageNumberLabel = tk.Label(self.frameMIDDLEIMGLABEL,
                                         text="Image Number:")
        ...
        self.skipToImageButton = tk.Button(self.frameMIDDLEIMGLABEL, text="Go",
        ...
        self.zoomOutButton = tk.Button(self.frameMIDDLEIMGZOOM, text="-",
                                       command=self.zoomOut, state=tk.DISABLED)
        self.zoomOutButton.pack(side=tk.LEFT)

        self.zoomInButton = tk.Button(self.frameMIDDLEIMGZOOM, text="+",
                                      command=self.zoomIn, state=tk.DISABLED)
        self.zoomInButton.pack(side=tk.LEFT)

        self.currentZoomLabel = tk.Label(self.frameMIDDLEIMGZOOM,
                                         text=' ' + str(
                                             self.imgScaleFactor) + 'X ')
        ...
        self.categoriesListbox = tk.Listbox(self.frameRIGHT,
    ...
    def initializeDatasets(self):
        ...
        # Get Datasets:
        d = './ds'
        self.dataSetsListDir = [os.path.join(d, o) for o in os.listdir(d) if
        ...
        for keyi in self.categories:
            if keyi[0] == "(" and keyi[2] == ")":
                if keyi[1] in self.keyBindings:
                    ...
                    print(
                        "ERROR! Multiple categories with the same key binding!")
                    exit()
                else:
    ...
    def loadImage(self):
        ...
        self.imgFileName.config(
            text=str(self.imageListStr[self.imageSelection]))
        ...
        for category_id in self.cursor.fetchall():
            self.cursor.execute(
                'SELECT categoryName FROM categories WHERE id = ?',
                (category_id[0],))
            ...
            try:
                ...
                print(
                    'FATAL ERROR! Image is saved with invalid category: ' + str(
                        categoryName))
                print('image Name: ' + self.imageListStr[self.imageSelection])
                print(
                    'This category must be listed in the categories.txt file.')
                exit()
    ...
    def saveImageCategorization(self):
        ...
        self.cursor.execute('DELETE FROM labels WHERE image_id = ?', (
    ...
    def loadDataSet(self):
        ...
        if self.dataSetSelection >= 0:
            ...
            self.imageListDir = [os.path.join(d, o) for o in os.listdir(d) if
                                 os.path.isdir(os.path.join(d, o)) == False]
            self.imageListStr = [o.split('\\')[-1] for o in self.imageListDir]
            self.dataSetStatusLabel.config(text='Dataset: ' + str(
            ...
            for imagei in range(len(self.imageListStr)):
                self.cursor.execute(
                    ...
                    (dataSet_id, str(self.imageListStr[imagei]),
            ...
            self.statusBar.config(text='Dataset loaded: ' + str(
                self.dataSetsListStr[
                    self.dataSetSelection]) + ' , Number of images: ' + str(
                len(self.imageListDir)))
            print('---->Dataset loaded: ' + str(self.dataSetsListStr[
                                                    self.dataSetSelection]) + ' , Number of images: ' + str(
                len(self.imageListDir)))
    ...
    def db_getImageID(self, imageStr):
        ...
        self.cursor.execute('SELECT id FROM images WHERE imagePath = ?',
                            (imageStr,))
        return self.cursor.fetchone()[0]

    def db_getCategoryID(self, categoryStr):
        # Get from database the category id
        self.cursor.execute('SELECT id FROM categories WHERE categoryName = ?',
                            (categoryStr,))
        return self.cursor.fetchone()[0]

    def db_getDataSetID(self, dataSetStr):
        # Select from database the dataset id
        self.cursor.execute('SELECT id FROM dataSets WHERE dataSetName = ?',
                            (dataSetStr,))
        return self.cursor.fetchone()[0]
